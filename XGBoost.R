...
head(data1)
drop_cols=c("Education_Level","Income_Category","Card_Category","Marital_Status","Gender")
...
set.seed(345)
model1=train(
  Attrition_Flag~.,data = data3,method = "xgbTree",
...
set.seed(345)
model2=train(
  Attrition_Flag~.,data = data4,method = "xgbTree",
  trControl = trainControl("cv",number = 10),
  tuneLength = 10
)
...
set.seed(345)
model3=train(
  Attrition_Flag~.,data = data1,method = "xgbTree",
  trControl = trainControl("cv",number = 10),
  tuneLength = 10
)
